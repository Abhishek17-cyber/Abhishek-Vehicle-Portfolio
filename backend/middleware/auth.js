...
async function verifyToken(req, res, next) {
  ...
  try {
    ...
    // Check database to ensure user still exists (invalidates deleted driver sessions immediately)
    const db = require('../config/db');
    const [rows] = await db.execute('SELECT id FROM users WHERE id = ?', [decoded.id]);
    if (!rows.length) {
