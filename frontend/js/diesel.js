  ...
  try {
    ...
    selects.forEach(sid => {
      ...
      if (!isDriver) {
        ...
        opt.value = '';
        opt.textContent = sid === 'filterVehicle' ? 'All Vehicles' : 'Select vehicle';
        sel.appendChild(opt);
      }
      ...
      if (isDriver && dieselVehicles.length > 0) {
        sel.value = dieselVehicles[0].id;
      }
    });
...
function clearDieselFilters() {
  ...
  const isDriver = user && user.role === 'driver';
  if (isDriver && dieselVehicles.length > 0) {
    document.getElementById('filterVehicle').value = dieselVehicles[0].id;
  } else {
...
function openAddDiesel() {
  ...
  const modalEl = document.getElementById('dieselModal');
