...
async function loadVehicleOptions() {
  ...
  try {
    ...
    if (filterSel) {
      ...
      if (isDriver && tripVehicles.length > 0) {
        filterSel.value = tripVehicles[0].id;
      }
    }
    ...
    if (modalSel) {
      ...
      if (isDriver && tripVehicles.length > 0) {
        modalSel.value = tripVehicles[0].id;
      }
    }
...
function clearFilters() {
  ...
  const isDriver = user && user.role === 'driver';
  if (isDriver && tripVehicles.length > 0) {
    document.getElementById('filterVehicle').value = tripVehicles[0].id;
  } else {
